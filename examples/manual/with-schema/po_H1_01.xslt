	...
	<xsl:template name="address">
		...
		<xsl:if test="phone">
			<div>
				...
				<!-- BAD EXAMPLE: this should trigger Rule H.1 because the input value ends up inside the JS handler without further precautions. -->
				<button onclick="window.open('tel:{phone}');">call customer</button>
			</div>
		</xsl:if>
	</xsl:template>
